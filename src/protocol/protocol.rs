 ...
 * 
 * UDP can also be forwarded in the WebSocket channel, by
 * mapping each unique UDP client address to a "UDP connection".
 * 
...
pub fn parse_packet<'a>(passwd: &str, packet: &'a [u8]) -> Packet<'a> {
    if let Ok(res) = handshake_parse(passwd, packet) {
        ...
        Packet::Connect(res)
    } else if let Ok(res) = udp_connect_parse(passwd, packet) {
        Packet::UdpConnect(res)
    } else if let Ok(res) = connect_state_parse(packet) {
        Packet::ConnectionState(res)
    } else if let Ok(res) = data_parse(packet) {
        Packet::Data(res)
    } else if let Ok(res) = udp_data_parse(packet) {
        Packet::UdpData(res)
    } else {
        Packet::Unrecognized
    }
...
/*
 * UDP Connect packet (authenticated)
 * Request to open a new logical UDP connection
 * to the remote (specified in the Handshake packet)
 * inside a WebSocket channel.
 * The server does not have to respond to this packet.
 * Though UDP is connectionless, TWS still needs to
 * distinguish between different clients by connection ID,
 * so that the server can connect to remote just as the
 * clients do. (i.e. server has to bind to different ports
 * for different "physical" UDP connections)
 * 
 * > AUTH [authentication code]
 * > NEW UDP CONNECTION [conn id]
 * 
 * Note that the TWS endpoints are free to drop any UDP
 * packet if the channel is full. This is different from
 * the TCP counterpart.
 * [conn id] should be a random 6-char string
 * generated by the client side.
 * TODO: Should we make authentication for this
 *  kind of packets more strict? i.e. include time
 */
pub fn udp_connect_build(passwd: &str, conn_id: &str) -> Result<String> {
    build_authenticated_packet(
        passwd,
        &format!("NEW UDP CONNECTION {}", conn_id)
    )
}

pub fn udp_connect_parse<'a>(passwd: &str, packet: &'a [u8]) -> Result<&'a str> {
    parse_authenticated_packet(passwd, packet)
        .and_then(|lines| {
            if lines.len() < 1 {
                return Err("Not a UDP Connect packet".into());
            }

            if !(lines[0].starts_with("NEW UDP CONNECTION ") && lines[0].len() == 25) {
                return Err("Not a UDP Connect packet".into());
            }
 ...
 * the remote / client.
 * 
 * > U[conn id][binary data]
 * 
 */
pub fn udp_data_build(conn_id: &str, data: &[u8]) -> Vec<u8> {
    [format!("U{}", conn_id).as_bytes(), data].concat()
}

pub fn udp_data_parse<'a>(packet: &'a [u8]) -> Result<(&'a str, &'a [u8])> {
    if packet.len() < 8 || packet[0] != "U".as_bytes()[0] {
        Err("Not a Data packet".into())
    } else {
        str::from_utf8(&packet[1..7])
            .chain_err(|| "Failed to decode conn_id")
...
mod tests {
    ...
    #[test]
    fn udp_connect_build_1() {
        assert_eq!(
            "AUTH OrR5xdwHsD7m6HuAmewHorGJOWPSDzRfr+MgxHqDqh0=\nNEW UDP CONNECTION XnjEa2",
            udp_connect_build("eeovgrg", "XnjEa2").unwrap()
        );
    }
    ...
    #[test]
    fn udp_data_parse_1() {
        assert_eq!(("acedef", "HelloWorld".as_bytes()), udp_data_parse("UacedefHelloWorld".as_bytes()).unwrap());
    }
}